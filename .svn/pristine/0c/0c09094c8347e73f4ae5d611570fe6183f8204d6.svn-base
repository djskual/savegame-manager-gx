...
void ManageButtons::SetAlignment(int h, int v)
{
    GuiElement::SetAlignment(h, v);
	buttons->SetAlignment(h, v);
}

void ManageButtons::SetButtonsEffect()
{
	move = windowinfo.move;
    
	if(windowinfo.move == ManageWindow::MOVE_IN)
		buttons->SetEffect(EFFECT_SLIDE_TOP | EFFECT_SLIDE_IN, 50);
	
	else if(windowinfo.move == ManageWindow::MOVE_OUT)
		buttons->SetEffect(EFFECT_SLIDE_TOP | EFFECT_SLIDE_OUT, 50);
	
	else if(windowinfo.move == ManageWindow::NO_MOVE_IN)
		buttons->SetEffect(EFFECT_FADE, 20);
	
    else if(windowinfo.move == ManageWindow::NO_MOVE_OUT)
		buttons->SetEffect(EFFECT_FADE, -20);
	
	else if(windowinfo.move == ManageWindow::MOVE_PREV_IN)
		buttons->SetEffect(EFFECT_SLIDE_LEFT | EFFECT_SLIDE_IN, 50);
	
    else if(windowinfo.move == ManageWindow::MOVE_NEXT_IN)
		buttons->SetEffect(EFFECT_SLIDE_RIGHT | EFFECT_SLIDE_IN, 50);
...
void ManageButtons::Update(GuiTrigger * t)
{
    ...
    if (btn1){
        if(btn1->GetState() == STATE_CLICKED) {
			...
			btn1->ResetState();
			change=true;
			windowinfo.move = ManageWindow::NO_MOVE_OUT;
	...
	if(change && move != windowinfo.move)
		SetButtonsEffect();

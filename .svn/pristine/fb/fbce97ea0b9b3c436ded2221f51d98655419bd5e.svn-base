 ...
 ***************************************************************************/
#ifndef _CEMUMIILIST_HPP_
#define _CEMUMIILIST_HPP_

#include <vector>

#include "../Text/wstring.hpp"

typedef struct _EmuMiiStruct
{
	std::string name;
	std::string creator;
	int position;
...
class CEmuMiiList
{
	public:
		CEmuMiiList() { };
		~CEmuMiiList() { ResetFull(); ResetFiltered(); };
		...
		int GetFullCount() { return EmuMiisFull.size(); };
		int GetFilteredCount() { return EmuMiisFiltered.size(); };
	...
	protected:
		...
		wString MiiFilter;
		std::string EmuMiiDBPath;
		
		std::vector<EmuMiiStruct *> EmuMiisFull;
		std::vector<EmuMiiStruct *> EmuMiisFiltered;
};

extern CEmuMiiList EmuMiiList;

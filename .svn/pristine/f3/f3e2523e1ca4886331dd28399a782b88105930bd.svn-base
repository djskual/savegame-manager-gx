...
/**
 * Constructor for the GuiCustomFilePathBrowser class.
 */
GuiCustomFilePathBrowser::GuiCustomFilePathBrowser(int w, int h)
{
	width = w;
	height = h;
	numEntries = 0;
	...
	customFolder = Resources::GetImageData(icon_folder_png, icon_folder_png_size);
...
/**
 * Destructor for the GuiCustomFilePathBrowser class.
 */
GuiCustomFilePathBrowser::~GuiCustomFilePathBrowser()
{
	delete arrowUpBtn;
	delete arrowDownBtn;
...
void GuiCustomFilePathBrowser::SetFocus(int f)
{
	focus = f;
...
void GuiCustomFilePathBrowser::ResetState()
{
	state = STATE_DEFAULT;
	stateChan = -1;
	selectedItem = 0;
...
void GuiCustomFilePathBrowser::TriggerUpdate()
{
	listChanged = true;
}
 ...
 */
void GuiCustomFilePathBrowser::Draw()
{
	if(!this->IsVisible())
		return;
...
void GuiCustomFilePathBrowser::Update(GuiTrigger * t)
{
	if(!t)
		return;
	...
	if(scrollbarBoxBtn->GetState() == STATE_HELD &&
		...
		t->wpad->ir.valid &&
		browserCFP.numEntries > CUSTOM_PAGESIZE
		)
	{
		...
		browserCFP.pageIndex = (positionWiimote * browserCFP.numEntries)/180 - selectedItem;

		if(browserCFP.pageIndex <= 0)
		{
			browserCFP.pageIndex = 0;
		}
		else if(browserCFP.pageIndex+CUSTOM_PAGESIZE >= browserCFP.numEntries)
		{
			browserCFP.pageIndex = browserCFP.numEntries-CUSTOM_PAGESIZE;
		}
		listChanged = true;
		focus = false;
	}
	...
	if(t->Right())
	{
		if(browserCFP.pageIndex < browserCFP.numEntries && browserCFP.numEntries > CUSTOM_PAGESIZE)
		{
			browserCFP.pageIndex += CUSTOM_PAGESIZE;
			if(browserCFP.pageIndex+CUSTOM_PAGESIZE >= browserCFP.numEntries)
				browserCFP.pageIndex = browserCFP.numEntries-CUSTOM_PAGESIZE;
			listChanged = true;
		}
	}
	...
	{
		if(browserCFP.pageIndex + selectedItem + 1 < browserCFP.numEntries)
		{
			if(selectedItem == CUSTOM_PAGESIZE-1)
			{
				// move list down by 1
				browserCFP.pageIndex++;
				listChanged = true;
			}
	...
	{
		if(selectedItem == 0 &&	browserCFP.pageIndex + selectedItem > 0)
		{
			// move list up by 1
			browserCFP.pageIndex--;
			listChanged = true;
  ...
  endNavigation:
	...
	for(int i=0; i<CUSTOM_PAGESIZE; i++)
	{
		if(listChanged || numEntries != browserCFP.numEntries)
		{
			if(browserCFP.pageIndex+i < browserCFP.numEntries)
			{
				...
				customListText[i]->SetText(browserCustomFilePath[browserCFP.pageIndex+i].displayname);
				
				if(browserCustomFilePath[browserCFP.pageIndex+i].isdir) {
		...
		{
			selectedItem = i;
			browserCFP.selIndex = browserCFP.pageIndex + i;
		}
	}
	...
	{
		position = 180*((browserCFP.pageIndex + CUSTOM_PAGESIZE)/2.0) / (browserCFP.numEntries*1.0);

		if(browserCFP.pageIndex/(CUSTOM_PAGESIZE/2.0) < 1)
			position = 0;
		else if((browserCFP.pageIndex+CUSTOM_PAGESIZE)/(CUSTOM_PAGESIZE*1.0) >= (browserCFP.numEntries)/(CUSTOM_PAGESIZE*1.0))
			position = 180;
	}
	...
	listChanged = false;
	numEntries = browserCFP.numEntries;

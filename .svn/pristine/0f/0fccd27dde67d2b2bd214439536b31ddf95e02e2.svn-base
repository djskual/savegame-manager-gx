...
/**
 * Constructor for the GuiCustomPathBrowser class.
 */
GuiCustomPathBrowser::GuiCustomPathBrowser(int w, int h)
{
	width = w;
	height = h;
	numEntries = 0;
...
/**
 * Destructor for the GuiCustomPathBrowser class.
 */
GuiCustomPathBrowser::~GuiCustomPathBrowser()
{
	delete arrowUpBtn;
	delete arrowDownBtn;
...
void GuiCustomPathBrowser::SetFocus(int f)
{
	focus = f;
...
void GuiCustomPathBrowser::ResetState()
{
	state = STATE_DEFAULT;
	stateChan = -1;
	selectedItem = 0;
...
void GuiCustomPathBrowser::TriggerUpdate()
{
	listChanged = true;
}
 ...
 */
void GuiCustomPathBrowser::Draw()
{
	if(!this->IsVisible())
		return;
...
void GuiCustomPathBrowser::Update(GuiTrigger * t)
{
	if(!t)
		return;
	...
	if(scrollbarBoxBtn->GetState() == STATE_HELD &&
		...
		t->wpad->ir.valid &&
		browserCP.numEntries > CUSTOM_PAGESIZE
		)
	{
		...
		browserCP.pageIndex = (positionWiimote * browserCP.numEntries)/180 - selectedItem;

		if(browserCP.pageIndex <= 0)
		{
			browserCP.pageIndex = 0;
		}
		else if(browserCP.pageIndex+CUSTOM_PAGESIZE >= browserCP.numEntries)
		{
			browserCP.pageIndex = browserCP.numEntries-CUSTOM_PAGESIZE;
		}
		listChanged = true;
		focus = false;
	}
	...
	if(t->Right())
	{
		if(browserCP.pageIndex < browserCP.numEntries && browserCP.numEntries > CUSTOM_PAGESIZE)
		{
			browserCP.pageIndex += CUSTOM_PAGESIZE;
			if(browserCP.pageIndex+CUSTOM_PAGESIZE >= browserCP.numEntries)
				browserCP.pageIndex = browserCP.numEntries-CUSTOM_PAGESIZE;
			listChanged = true;
		}
	}
	...
	{
		if(browserCP.pageIndex + selectedItem + 1 < browserCP.numEntries)
		{
	...
	{
		if(selectedItem == 0 &&	browserCP.pageIndex + selectedItem > 0)
		{
			// move list up by 1
			browserCP.pageIndex--;
			listChanged = true;
  ...
  endNavigation:
	...
	for(int i=0; i<CUSTOM_PAGESIZE; i++)
	{
		if(listChanged || numEntries != browserCP.numEntries)
		{
			if(browserCP.pageIndex+i < browserCP.numEntries)
			{
				...
				customListText[i]->SetText(browserCustomPath[browserCP.pageIndex+i].displayname);
		...
		{
			selectedItem = i;
			browserCP.selIndex = browserCP.pageIndex + i;
		}
	}
	...
	{
		position = 180*((browserCP.pageIndex + CUSTOM_PAGESIZE)/2.0) / (browserCP.numEntries*1.0);

		if(browserCP.pageIndex/(CUSTOM_PAGESIZE/2.0) < 1)
			position = 0;
		else if((browserCP.pageIndex+CUSTOM_PAGESIZE)/(CUSTOM_PAGESIZE*1.0) >= (browserCP.numEntries)/(CUSTOM_PAGESIZE*1.0))
			position = 180;
	}
	...
	listChanged = false;
	numEntries = browserCP.numEntries;

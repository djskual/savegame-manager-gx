...
/**
 * Constructor for the ListSaveBrowser class.
 */
ListSaveBrowser::ListSaveBrowser(Browser * filebrowser, GuiImageData * browserBg, int w, int h)
...
/**
 * Destructor for the ListSaveBrowser class.
 */
ListSaveBrowser::~ListSaveBrowser()
{
	browser = NULL;
...
void ListSaveBrowser::AddButton()
{
...
void ListSaveBrowser::RemoveButton(int i)
{
...
void ListSaveBrowser::SetButton(int i, std::string name, bool enable)
{
	if(i < 0)
		return;
...
GuiImage * ListSaveBrowser::GetTypeIcon()
{
	switch (type)
	{
		case TYPE_WII:
...
void ListSaveBrowser::FoundSaveType()
{
	type = TYPE_UNKNOW;
	
	std::string ID = browser->GetItemSaveID(browser->GetPageIndex()+selectedItem);
	if(ID.empty())
		return;
...
GuiImage * ListSaveBrowser::GetSaveIcon()
{	
	u64 tid = browser->GetItemSaveTid(browser->GetPageIndex()+selectedItem);
...
void ListSaveBrowser::SetSelected(int i)
{
	if(i < 0 || i >= (int) saveBtn.size())
		return;
...
void ListSaveBrowser::ResetState()
{
	state = STATE_DEFAULT;
	stateChan = -1;
	selectedItem = 0;
...
void ListSaveBrowser::SetSelectedInfos()
{
	...
	infoNameTxt = new GuiText(browser->GetItemSaveName(browser->GetPageIndex()+selectedItem), 17, thColor("r=0 g=0 b=0 a=255 - mainbrowser entry text color"));
	...
	infoSubnameTxt = new GuiText(browser->GetItemSaveSubname(browser->GetPageIndex()+selectedItem), 15, thColor("r=0 g=0 b=0 a=255 - mainbrowser entry text color"));
	...
	std::string ID = tr( "ID:" );
	ID += " ";
	ID += browser->GetItemSaveID(browser->GetPageIndex()+selectedItem);
	...
	size += fmt(" %0.f %s", ceil(browser->GetItemSaveBlocks(browser->GetPageIndex()+selectedItem)), tr( "Block(s)" ));
...
std::string ListSaveBrowser::GetCurrentPath()
{
	std::string CurrentPath;
	
	u64 tid = browser->GetItemSaveTid(browser->GetPageIndex()+selectedItem);
	CurrentPath = fmt("isfs:/title/%08x/%08x/data",(u32)(tid >> 32), (u32)(tid & 0xFFFFFFFF));
...
void ListSaveBrowser::Update(GuiTrigger * t)
{
	if(state == STATE_DISABLED || !t || !triggerupdate)
		return;

	if(!browser->GetSaveListCount())
	{
	...
	if(t->Right())
	{
		if(browser->GetPageIndex() < browser->GetSaveListCount() && browser->GetSaveListCount() > PAGESIZE)
		{
			int pageIndex = browser->GetPageIndex() + PAGESIZE;
			if(pageIndex+PAGESIZE >= browser->GetSaveListCount())
				pageIndex = browser->GetSaveListCount()-PAGESIZE;
	...
	{
		if(browser->GetPageIndex() + selectedItem + 1 < browser->GetSaveListCount())
		{
	...
	if(numEntries != browser->GetSaveListCount())
	{
		numEntries = browser->GetSaveListCount();
	...
	for(int i = 0; i < PAGESIZE; i++)
	{
		...
		if(listChanged)
		{
			if(browser->GetPageIndex()+i < browser->GetSaveListCount())
			{
				std::string  name = browser->GetItemSaveName(browser->GetPageIndex()+i);

...
u32 Nand::GetFileSize(int fp)
{
	...
	fstats *stats = (fstats *)Tools::AllocateMemory_64(sizeof(fstats));
...
int Nand::ReadDir(std::string path, std::vector<std::string> &list)
{
	...
	char *dirlist = (char*)Tools::AllocateMemory_64(ISFS_MAXPATH * count);
	if (!dirlist)
		return ISFS_ENOMEM;
...
int Nand::LoadFile(std::string filepath, u8 **outbuffer, u32 *outfilesize)
{
	...
	if (file >= 0)
	{
		...
		if (filesize)
		{
			buffer = (u8 *)Tools::AllocateMemory_32(filesize);
			if (buffer)
			{

...
u8 * GDImageToRGBA8(gdImagePtr * gdImg, int * w, int * h)
{
    ...
    for(y = 0; y < (u32) height; ++y)
    {
        for(x = 0; x < (u32) width; ++x)
        {
            ...
            a = 254 - 2*(gdImageAlpha(*gdImg, (int)pixel) & 0xFF);
            if(a == 254) a++;

            offset = coordsRGBA8(x, y, width);
            data[offset] = a;
            data[offset+1] = (u8)gdImageRed(*gdImg, (int)pixel);
            data[offset+32] = (u8)gdImageGreen(*gdImg, (int)pixel);
            data[offset+33] = (u8)gdImageBlue(*gdImg, (int)pixel);
        }
    }

...
GuiSound *Resources::GetSound(const u8 *snd, s32 sndSize, int volume)
{
	return Resources::Instance()->InternalGetSound(snd, sndSize, volume);
}
...
GuiSound *Resources::InternalGetSound(const u8 *snd, s32 sndSize, int volume)
{
	...
	if (itr == sounds.end())
	{
		...
		GuiSound *d = new GuiSound(snd, sndSize, volume);
		sounds[snd] = d;
		soundCount[d] = 1;
		return d;

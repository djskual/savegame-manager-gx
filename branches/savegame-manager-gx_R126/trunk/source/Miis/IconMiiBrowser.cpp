 ...
 */
IconMiiBrowser::IconMiiBrowser(Browser * filebrowser, GuiImageData * browserBg, int w, int h)
...
void IconMiiBrowser::SetButton(int i, std::string name, bool enable, int x, int y)
{
	if(i < 0)
		return;
...
void IconMiiBrowser::SetSelected(int i)
{
	if(i < 0 || i >= PageSize)
		return;
...
void IconMiiBrowser::ResetState()
{
	state = STATE_DEFAULT;
	stateChan = -1;
	selectedItem = 0;
...
void IconMiiBrowser::Draw()
{
	...
	if(parentElement && parentElement->GetState() != STATE_DISABLED)
	{
		...
		bool tooltip = false;
		if (SelectTimer.elapsed() > 2 && Settings.Tooltips && browser->GetMiiCount())
		{
...
void IconMiiBrowser::Update(GuiTrigger * t)
{
	...
	{
		int i = RowSize;
		while(browser->GetPageIndex()+selectedItem+RowSize >= browser->GetMiiCount() &&
			  i > 0 && selectedItem > 0 && RowSize < browser->GetMiiCount())
		{
			--i;
			--selectedItem;
		}
		if(browser->GetPageIndex() + selectedItem + RowSize < browser->GetMiiCount())
		{
			Buttons[selectedItem]->ResetState();
			selectedItem = selectedItem+RowSize;

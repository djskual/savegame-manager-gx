...
/**
 * Constructor for the ListMiiBrowser class.
 */
ListMiiBrowser::ListMiiBrowser(Browser * filebrowser, GuiImageData * browserBg, int w, int h)
...
/**
 * Destructor for the ListMiiBrowser class.
 */
ListMiiBrowser::~ListMiiBrowser()
{
	browser = NULL;
...
void ListMiiBrowser::RemoveButton(int i)
{
...
void ListMiiBrowser::SetButton(int i, std::string name, bool enable)
{
	if(i < 0)
		return;
...
void ListMiiBrowser::SetSelected(int i)
{
	if(i < 0 || i >= (int) miiBtn.size())
		return;
...
void ListMiiBrowser::ResetState()
{
	state = STATE_DEFAULT;
	stateChan = -1;
	selectedItem = 0;
...
std::string ListMiiBrowser::GetDate()
{
	std::string Date =  "..";
	
	if (browser->GetItemMiiDay(browser->GetPageIndex()+selectedItem) != 0)
		Date = fmt("%d", browser->GetItemMiiDay(browser->GetPageIndex()+selectedItem));
	Date += " / ";
	
	switch (browser->GetItemMiiMonth(browser->GetPageIndex()+selectedItem))
	{
		case 0: { Date += ".."; break; }
...
GXColor ListMiiBrowser::GetFavColor()
{
	GXColor color = (GXColor) {0, 0, 0, 0};
	
	switch (browser->GetItemMiiFavColor(browser->GetPageIndex()+selectedItem))
	{
...
void ListMiiBrowser::SetSelectedInfos()
{
	...
	infoNameTxt = new GuiText(browser->GetItemMiiName(browser->GetPageIndex()+selectedItem), 17, thColor("r=0 g=0 b=0 a=255 - mainbrowser entry text color"));
	...
	Creator += " ";
	if(browser->GetItemMiiCreator(browser->GetPageIndex()+selectedItem).size())
		Creator += browser->GetItemMiiCreator(browser->GetPageIndex()+selectedItem);
	else
		Creator += "...";
	...
	infoGenderImg = new GuiImage(browser->GetItemMiiFemale(browser->GetPageIndex()+selectedItem) ? femaleImgData : maleImgData);
	...
	int Y = -40;
	infoPathTxt = new GuiText("isfs:/shared2/menu/FaceLib/RFL_DB.dat", 16, thColor("r=0 g=0 b=0 a=255 - mainbrowser entry text color"));
...
void ListMiiBrowser::Update(GuiTrigger * t)
{
	if(state == STATE_DISABLED || !t || !triggerupdate || !browser->GetMiiCount())
		return;

	if(!browser->GetMiiCount())
	{
	...
	if(t->Right())
	{
		if(browser->GetPageIndex() < browser->GetMiiCount() && browser->GetMiiCount() > PAGESIZE)
		{
			int pageIndex = browser->GetPageIndex() + PAGESIZE;
			if(pageIndex+PAGESIZE >= browser->GetMiiCount())
	...
	{
		if(browser->GetPageIndex() + selectedItem + 1 < browser->GetMiiCount())
		{
	...
	if(numEntries != browser->GetMiiCount())
	{
		numEntries = browser->GetMiiCount();
	...
	for(int i = 0; i < PAGESIZE; i++)
	{
		...
		if(listChanged)
		{
			if(browser->GetPageIndex()+i < browser->GetMiiCount())
			{
				std::string  name = browser->GetItemMiiName(browser->GetPageIndex()+i);

...
{
	char filepath[300];
	snprintf(filepath, sizeof(filepath), "%s/theme_preview.png", ThemeList[theme].ImageFolder.c_str());
...
void ThemeMenu::MainButtonClicked(int button)
{
	...
	GuiButton applyBtn(&applyBtnImg, &applyBtnImg, ALIGN_RIGHT, ALIGN_TOP, -5, 170, &trigA, btnSoundOver, btnSoundClick, 1);
	applyBtn.SetLabel(&applyBtnTxt);
	applyBtn.SetScale(0.8);

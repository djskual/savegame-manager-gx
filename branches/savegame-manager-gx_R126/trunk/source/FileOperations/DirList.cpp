...
DirList::DirList(std::string path, const char *filter, u32 flags)
{
...
bool DirList::LoadPath(std::string &folderpath, const char *filter, u32 flags)
{
	...
	while ((dirent = readdir(dir)) != 0)
	{
		...
		snprintf(filename, MAXPATHLEN, isRoot ? "%s%s" : "%s/%s", folderpath.c_str(), dirent->d_name);
	...
	for (u32 i = 0; i < FolderList.size() && !homebrewFound; i++)
		LoadPath(FolderList.at(i), filter, flags);
...
void DirList::AddEntrie(const char * folderpath, const char * filename, u64 filesize, bool isDir)
{
	...
	if(FileInfo[pos].FilePath)
	{
		...
		if(folder[folder.size()-1] == '/')
			folder.erase(folder.size()-1, 1);
		
		sprintf(FileInfo[pos].FilePath, "%s/%s", folder.c_str(), filename);
	}
	FileInfo[pos].FileSize = filesize;
	FileInfo[pos].isDir = isDir;
}

void DirList::RemoveEntrie(int index)
{
	FileInfo.erase(FileInfo.begin()+index);
}

...
MiiWindow::MiiWindow(int Selected)
	: GuiWindow(572, 372)
{
	returnVal = -1;
	...
	miiInfos = new MiiInfos(miiSelected, BROWSE_MII);
...
void MiiWindow::SetWindowEffect(int direction, int in_out)
{
	if(direction == LEFT) {
		if(in_out == IN)
...
void MiiWindow::RemoveEntry()
{
	MiiList.RemoveEntry(miiSelected);
	...
	miiSelected--;
	if (miiSelected < 0)
		miiSelected = MiiList.GetFilteredCount()-1;
	
	ChangeMii(RIGHT);
}

void MiiWindow::ChangeMii(int EffectDirection)
{
	SetWindowEffect(EffectDirection, OUT);
	
	HaltGui();
	
	nameTxt->SetText(MiiList.GetName(miiSelected).c_str());
...
void MiiWindow::SetButtonsClickable(bool clickable)
{
	if ( !clickable )
	{
...
int MiiWindow::Show()
{
	...
	if(refreshNeeded)
	{
		wString oldFilter(MiiList.GetCurrentFilter());
		MiiList.FilterList(oldFilter.c_str());
	}
	
	return choice;
}

int MiiWindow::MainLoop()
{
	...
	{
		miiSelected++;
		if (miiSelected >= MiiList.GetFilteredCount())
			miiSelected = 0;
	...
	else if (actionsButtons->GetState() == STATE_CLICKED)
	{
		...
		else if (action == DELETE_NAND_MII || action == INSTALL_EMU_MII)
		{
			if(action == INSTALL_EMU_MII)
			{
	...
	else if (confirmButtons->GetState() == STATE_CLICKED)
	{
		...
		if (choice == 1 && action == EXTRACT_MII)
		{
			Tools = new MiiManageTools(tr("Extracting..."));
			
			Tools->ExtractMii_Nand_Dev(Settings.TmpPath, miiSelected);
			...
			else if (Tools->GetResult() == -10)
			{
				...
				//RemoveFile((char*)Settings.TmpPath.c_str());
			}
		...
		else if (choice == 1 && action == DELETE_NAND_MII)
		{
			Tools = new MiiManageTools(tr( "Deleting..." ));
			
			Tools->DeleteMii_Nand(MiiList.GetPosition(miiSelected));
		...
		else if (choice == 1 && action == INSTALL_EMU_MII)
		{
			...
			for(int n = 0; n < EmuMiiList.GetFullCount(); n++)
			{
				if(!EmuMiiList.GetFullName(n).compare(MiiList.GetName(miiSelected)))
				{
					Tools->StopProgress();
					Tools->Reset();
					delete Tools;
					...
					Tools = new MiiManageTools(tr( "Installing..." ));
					n = EmuMiiList.GetFullCount();
				}
			}
			
			if(!overwriteOk)
				position = Tools->FindFirstFree(false);
			Tools->InstallMii_Nand_Emu(MiiList.GetPosition(miiSelected), position);

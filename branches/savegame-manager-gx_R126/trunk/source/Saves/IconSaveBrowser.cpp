...
/**
 * Constructor for the IconSaveBrowser class.
 */
IconSaveBrowser::IconSaveBrowser(Browser * savebrowser, GuiImageData * browserBg, int w, int h)
	: GuiFileBrowser(savebrowser, w, h)
{
	width = w;
	height = h;
	numEntries = 0;
...
/**
 * Destructor for the IconSaveBrowser class.
 */
IconSaveBrowser::~IconSaveBrowser()
{
	HaltGui();
...
void IconSaveBrowser::AddButton()
{
	//! File Icon
...
void IconSaveBrowser::SetButton(int i, std::string name, bool enable, int x, int y)
{
	if(i < 0)
		return;
...
void IconSaveBrowser::RemoveButton(int i)
{
...
void IconSaveBrowser::SetSelected(int i)
{
	if(i < 0 || i >= PageSize)
		return;
...
void IconSaveBrowser::ResetState()
{
	state = STATE_DEFAULT;
	stateChan = -1;
	selectedItem = 0;
...
void IconSaveBrowser::Update(GuiTrigger * t)
{
	...
	{
		int i = RowSize;
		while(browser->GetPageIndex()+selectedItem+RowSize >= browser->GetSaveListCount() &&
			  i > 0 && selectedItem > 0 && RowSize < browser->GetSaveListCount())
		{
			--i;
			--selectedItem;
		}
		if(browser->GetPageIndex() + selectedItem + RowSize < browser->GetSaveListCount())
		{
			Buttons[selectedItem]->ResetState();
			selectedItem = selectedItem+RowSize;

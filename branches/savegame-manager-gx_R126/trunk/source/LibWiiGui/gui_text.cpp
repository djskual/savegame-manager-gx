...
#define TEXT_SCROLLWIDE_INITIAL_DELAY	2
...
GuiText::GuiText(std::string t, int s, GXColor c)
{
	text = NULL;
	size = s;
	currentSize = size;
...
void GuiText::SetMaxWidth(int width, int w)
{
	...
	{
		textScrollPos = 0;
		textScrollInitialDelay = TEXT_SCROLLWIDE_INITIAL_DELAY;
		textScrollDelay = TEXT_SCROLL_DELAY;
...
void GuiText::SetInterline(int i)
{
	interline = i;
}
...
void GuiText::MakeWideText()
{
	int spaceWidth = (font ? font : fontSystem)->getWidth(L" ", currentSize);
	int currentWidth = spaceWidth;
	...
	wString *ws_text = new wString(text);
	std::string str_text = ws_text->toUTF8();
	delete ws_text;
	
	std::string final_text = space;
	final_text += str_text;
	final_text += space;
	
	SetText(final_text.c_str());
}
...
{
	wchar_t temptext[wcslen(text)+1];
	wcscpy(temptext, text);
	if(wrapMode != SCROLL_WIDE)
		wcscat(temptext, L"   ");
	...
	if (textDyn.empty())
	{
		...
		while (temptext[i] && currentWidth < maxWidth)
		{
			textDyn[pos][i] = temptext[i];
			
			currentWidth += (font ? font : fontSystem)->getCharWidth(temptext[i], currentSize, i > 0 ? temptext[i - 1] : 0);
	...
	while (currentWidth < maxWidth)
	{
		...
		textDyn[pos][i] = temptext[ch];
		++ch;
		++i;
		
		currentWidth += (font ? font : fontSystem)->getCharWidth(temptext[ch], currentSize, ch > 0 ? temptext[ch - 1] : 0x0000);
	}
	textDyn[pos][i] = 0;
}
...
void GuiText::Draw()
{
	...
	if (maxWidth > 0 && maxWidth <= textWidth)
	{
		...
		else if (wrapMode == SCROLL_HORIZONTAL || wrapMode == SCROLL_WIDE)
		{
			ScrollText();

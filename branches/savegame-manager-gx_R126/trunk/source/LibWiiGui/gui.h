...
class GuiText: public GuiElement
{
	public:
		...
		GuiText(std::string t, int s, GXColor c);
		//!\overload
		...
		virtual void SetText(std::string t) { return SetText(t.c_str()); };
	...
	protected:
		...
		//!Create a dynamic scrollwide text
		void MakeWideText();
...
{
	public:
		GuiKeyboard(char * t, u32 max, int minimum, int lang);
		virtual ~GuiKeyboard();
		void Update(GuiTrigger * t);
		char kbtextstr[256];
	protected:
...
//!Display, manage, and manipulate BlockBox in the GUI.
class GuiBlockBox: public GuiElement
{
	public:
		//!Constructor
		//!\param t Text
		GuiBlockBox(const char *t, int Alpha = 255);
		//!Destructor
		virtual ~GuiBlockBox();
		//!Gets the element's current scale
		//!Considers scale, scaleDyn, and the parent element's GetScale() value
	...
	protected:
		GuiImageData * blockboxSide;
		GuiImageData * blockboxTile;
		GuiImage * leftImage; //!< Tooltip left-image
		GuiImage * tileImage; //!< Tooltip tile-image
		GuiImage * rightImage; //!< Tooltip right-image
		GuiText *text;

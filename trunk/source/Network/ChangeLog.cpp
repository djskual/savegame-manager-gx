...
ChangeLog::ChangeLog(UpdateWindow * update_window)
	: newWindow(false)
...
char * ChangeLog::GetChangeLogRange(int fromRev, int tillRev, bool backwards)
{
	...
	while(RevCounter != StopRev)
	{
		...
		updateWindow->ShowProgress(tr("Getting Changelog..."), ProgressText, done, 6);
...
bool ChangeLog::Show()
{
	...
	if(GetSavedChangeLogRev() < GetIntRev())
	{
		if(!DownloadChangeLog(GetIntRev()-5, GetIntRev()))
			return false;
	}
	...
	TextEditor::LoadFile(changelogpath, false);

...
int Playlog::Update(const char * ID, const u16 * title)
{
	...
	PlayRec * playrec_buf = (PlayRec *)MemTools::AllocateMemory_32(sizeof(PlayRec)); //! Should be 32 byte aligned
	if(playrec_buf)
	{
...
int Playlog::Delete()
{
	...
	if(fd >= 0)
	{
		PlayRec * playrec_buf = (PlayRec *)MemTools::AllocateMemory_32(sizeof(PlayRec));
		if(playrec_buf)
		{
			//!Read play_rec.dat

...
void Resources::Clear()
{
	...
	for (imgitr = Resources::Instance()->images.begin(); imgitr != Resources::Instance()->images.end(); imgitr++)
	{
		delete imgitr->second;
	}
	Resources::Instance()->images.clear();

	// Delete all sounds...
	std::map<const u8 *, GuiSound *>::iterator snditr;
	for (snditr = Resources::Instance()->sounds.begin(); snditr != Resources::Instance()->sounds.end(); snditr++)
	{
		delete snditr->second;
	}
	Resources::Instance()->sounds.clear();

	Resources::Instance()->imageCount.clear();
	Resources::Instance()->soundCount.clear();
}
...
bool Resources::LoadFiles(const char * path)
{
	...
	{
		snprintf(fullpath, sizeof(fullpath), "%s/%s", path, RecourceList[i].filename);

...
void GuiImage::Draw()
{
	...
	if(image && tileHorizontal > 0 && tileVertical > 0)
	{
		for(int n=0; n<tileVertical; n++)
			...
			{
				if(bUnCut)
					Menu_DrawImg(image, width, height, format, currLeft+width*i, currTop+height*n, currZ, imageangle, currScaleX, currScaleY, currAlpha);
				else
					Menu_DrawImgCut(image, width, height, format, currLeft+width*i, currTop+height*n, currZ, imageangle, currScaleX, currScaleY, currAlpha, cutBoundsRect.x1(), cutBoundsRect.x2(), cutBoundsRect.y1(), cutBoundsRect.y2());
			}
	}
